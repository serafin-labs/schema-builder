...
export class SchemaBuilder<T> {
  ...
  constructor(
    ...
    protected validationConfig?: Ajv.Options
  ) {
    throughJsonSchema(this.schemaObject, (s) => {
      if ("$ref" in s) {
        throw new VError(
          `Schema Builder Error: $ref can't be used to initialize a SchemaBuilder. Dereferenced the schema first.`
        );
      }
    });
  }
   ...
   */
  static enumSchema<K extends keyof any, N extends boolean = false>(
    values: K[],
    schema: Pick<JSONSchema, JSONSchemaProperties> = {},
    nullable?: boolean
   ...
   */
  static oneOf<S extends SchemaBuilder<any>[]>(
    ...schemaBuilders: S
  ): SchemaBuilder<OneOf<S>> {
   ...
   */
  static allOf<S extends SchemaBuilder<any>[]>(
    ...schemaBuilders: S
  ): SchemaBuilder<AllOf<S>> {
    return new SchemaBuilder<any>({
   ...
   */
  static anyOf<S extends SchemaBuilder<any>[]>(
    ...schemaBuilders: S
  ): SchemaBuilder<OneOf<S>> {
    return new SchemaBuilder<any>({
   ...
   */
  setOptionalProperties<K extends keyof T>(
    properties: K[]
  ): SchemaBuilder<PartialProperties<T, K>> {
   ...
   */
  setRequiredProperties<K extends keyof T>(
    properties: K[]
  ): SchemaBuilder<RequiredProperties<T, K>> {
    if (!this.isSimpleObjectSchema) {
      throw new VError(
    ...
    for (let propertyName in schemaObject.properties) {
      if (required.indexOf(propertyName) === -1) {
        ...
        if (typeof propertyValue !== "boolean" && "type" in propertyValue) {
          if (
            Array.isArray(propertyValue.type) &&
            propertyValue.type.indexOf("null") === -1
          ) {
          ...
          if (
            ...
            propertyValue.enum.indexOf(null) === -1
          ) {
  ...
  addProperty<U, K extends keyof any, REQUIRED extends boolean = true>(
    ...
    isRequired?: REQUIRED
  ): SchemaBuilder<Combine<T, U, K, REQUIRED, false>> {
    if (!this.isObjectSchema) {
      throw new VError(
        `Schema Builder Error: you can only add properties to an object schema`
      );
    }
    let schemaObject = cloneJSON(this.schemaObject);
    ...
    if (propertyName in schemaObject.properties) {
      throw new VError(
        `Schema Builder Error: '${propertyName}' already exists in ${
          schemaObject.title || "this"
        } schema`
      );
    }
    schemaObject.properties[propertyName as string] = cloneJSON(
      schemaBuilder.schemaObject
    );
  ...
  replaceProperty<U, K extends keyof T, REQUIRED extends boolean = true>(
    ...
    if (!this.isObjectSchema) {
      throw new VError(
        `Schema Builder Error: you can only replace properties of an object schema`
      );
    }
    let schemaObject = cloneJSON(this.schemaObject);
    schemaObject.properties = schemaObject.properties || {};
   ...
   */
  addAdditionalProperties<U = any>(
    schemaBuilder?: SchemaBuilder<U>
  ): SchemaBuilder<T & { [P: string]: U }> {
    if (this.schemaObject.additionalProperties) {
      throw new VError(
        `Schema Builder Error: additionalProperties is already set in ${
          this.schemaObject.title || "this"
        } schema.`
      );
    }
    let schemaObject = cloneJSON(this.schemaObject);
    schemaObject.additionalProperties = schemaBuilder
  ...
  >(
    ...
    isRequired?: REQUIRED,
    nullable?: N
  ): SchemaBuilder<Combine<T, string, K, REQUIRED, N>> {
    return this.addProperty(
      propertyName,
      SchemaBuilder.stringSchema(schema, nullable),
      isRequired
    ) as any;
  }
  ...
  >(
    ...
    return this.addProperty(
      propertyName,
      SchemaBuilder.enumSchema(values, schema, nullable),
      isRequired
    ) as any;
  }
  ...
  >(
    ...
    isRequired?: REQUIRED,
    nullable?: N
  ): SchemaBuilder<Combine<T, number, K, REQUIRED, N>> {
    return this.addProperty(
      propertyName,
      SchemaBuilder.numberSchema(schema, nullable),
      isRequired
    ) as any;
  }
  ...
  >(
    ...
    return this.addProperty(
      propertyName,
      SchemaBuilder.integerSchema(schema, nullable),
      isRequired
    ) as any;
  }
  ...
  >(
    ...
    isRequired?: REQUIRED,
    nullable?: N
  ): SchemaBuilder<Combine<T, boolean, K, REQUIRED, N>> {
    return this.addProperty(
      propertyName,
      SchemaBuilder.booleanSchema(schema, nullable),
      isRequired
    ) as any;
  }
  ...
  >(
    ...
    isRequired?: REQUIRED,
    nullable?: N
  ): SchemaBuilder<Combine<T, U[], K, REQUIRED, N>> {
    return this.addProperty(
      propertyName,
      SchemaBuilder.arraySchema(items, schema, nullable),
      isRequired
    ) as any;
  }
  ...
  renameProperty<K extends keyof T, K2 extends keyof any>(
    ...
    schemaObject.properties = schemaObject.properties || {};
    if (propertyName in schemaObject.properties) {
      schemaObject.properties[newPropertyName as string] =
        schemaObject.properties[propertyName as string];
      delete schemaObject.properties[propertyName as string];
      // rename the property in the required array if needed
      if (
        schemaObject.required &&
        schemaObject.required.indexOf(propertyName as string) !== -1
      ) {
        schemaObject.required.splice(
          schemaObject.required.indexOf(propertyName as string),
          1
        );
   ...
   */
  pickProperties<K extends keyof T>(
    properties: K[]
  ): SchemaBuilder<{ [P in K]: T[P] }> {
    if (!this.isObjectSchema || this.hasSchemasCombinationKeywords) {
      throw new VError(
        `Schema Builder Error: 'pickProperties' can only be used with a simple object schema (no oneOf, anyOf, allOf or not)`
      );
    }
    let schemaObject = cloneJSON(this.schemaObject);
    schemaObject.properties = schemaObject.properties || {};
    ...
    if (schemaObject.required) {
      schemaObject.required = schemaObject.required.filter(
        (r: string) => (properties as string[]).indexOf(r) !== -1
      );
    }
    if (
      Array.isArray(schemaObject.required) &&
      schemaObject.required.length === 0
    ) {
      delete schemaObject.required;
    }
  ...
  pickAdditionalProperties<K extends keyof T, K2 extends keyof T = any>(
    ...
    if (!additionalProperties) {
      schemaObject.additionalProperties = additionalProps
        ? additionalProps
        : true;
    } else if (
      ...
      for (let additionalProperty of additionalProperties) {
        schemaObject.properties[additionalProperty] =
          typeof additionalProps === "boolean"
            ? {}
            : cloneJSON(additionalProps);
   ...
   */
  omitProperties<K extends keyof T>(
    properties: K[]
    ...
    let p = Object.keys(this.schemaObject.properties || {}).filter(
      (k) => (properties as string[]).indexOf(k) === -1
    );
  ...
  transformProperties<U, K extends keyof T>(
    schemaBuilder: SchemaBuilder<U>,
    propertyNames?: K[]
  ): SchemaBuilder<TransformProperties<T, K, U>> {
    if (!this.isSimpleObjectSchema) {
      throw new VError(
    ...
    schemaObject.properties = schemaObject.properties || {};
    propertyNames =
      propertyNames || (Object.keys(schemaObject.properties) as K[]);
  ...
  transformPropertiesToArray<K extends keyof T>(
    propertyNames?: K[]
  ): SchemaBuilder<TransformPropertiesToArray<T, K>> {
    if (!this.isSimpleObjectSchema) {
      throw new VError(
    ...
    for (let property of propertyNames) {
      ...
      if ((propertySchema as JSONSchema).type !== "array") {
        schemaObject.properties[property as string] = {
          oneOf: [
            propertySchema,
            { type: "array", items: cloneJSON(propertySchema) },
          ],
        };
      }
    }
  ...
  unwrapArrayProperties<K extends keyof T>(
    propertyNames?: K[]
  ): SchemaBuilder<UnwrapArrayProperties<T, K>> {
    if (!this.isSimpleObjectSchema) {
      throw new VError(
      ...
      for (let propertyKey in schemaObject2.properties) {
        if (!(propertyKey in schemaObject1.properties)) {
          schemaObject1.properties[propertyKey] =
            schemaObject2.properties[propertyKey];
          if (
      ...
      for (let propertyKey in schemaObject2.properties) {
        if (!(propertyKey in schemaObject1.properties)) {
          ...
          if (
            ...
            schemaObject1.required = schemaObject1.required.filter(
              (p: string) => p !== propertyKey
            );
          }
        }
      }
   ...
   * This method only copy properties.
   */
  overwriteProperties<T2>(
    schema: SchemaBuilder<T2>
  ): SchemaBuilder<Overwrite<T, T2>> {
    if (!this.isSimpleObjectSchema) {
      throw new VError(
  ...
  getSubschema<K extends keyof T>(propertyName: K) {
    if (
      !this.isSimpleObjectSchema ||
      !this.schemaObject ||
      typeof this.schemaObject === "boolean" ||
      !this.schemaObject.properties
    ) {
      throw new VError(
        `Schema Builder Error: 'extract' can only be used with a simple object schema (no additionalProperties, oneOf, anyOf, allOf or not)`
      );
    } else {
      return new SchemaBuilder<T[K]>(
        this.schemaObject.properties[propertyName as string] as JSONSchema
      );
    }
  }
  ...
  get isObjectSchema() {
    return (
      this.schemaObject.type === "object" ||
      (!("type" in this.schemaObject) && "properties" in this.schemaObject)
    );
  }
  ...
  get hasAdditionalProperties() {
    return (
      this.isObjectSchema && this.schemaObject.additionalProperties !== false
    );
  }
  ...
  get hasSchemasCombinationKeywords() {
    return (
      "oneOf" in this.schemaObject ||
      "allOf" in this.schemaObject ||
      "anyOf" in this.schemaObject ||
      "not" in this.schemaObject
    );
  }
   ...
   */
  setSchemaAttributes(
    schema: Pick<JSONSchema, JSONSchemaGeneralProperties>
  ...
  validate(o: T) {
    ...
    if (!valid) {
      throw validationError(
        this.ajv.errorsText(this.validationFunction.errors),
        this.validationFunction.errors
      );
    }
  }
  protected ajv: any;
  ...
  validateList(list: T[]) {
    ...
    if (!valid) {
      throw validationError(
        this.ajvList.errorsText(this.listValidationFunction.errors),
        this.listValidationFunction.errors
      );
    }
  }
...
function throughJsonSchema(
  schema: JSONSchema | JSONSchema[],
  action: (schema: JSONSchema) => void
) {
...
export type JSONSchemaArrayProperties =
  | "description"
  | "default"
  | "maxItems"
  | "minItems"
...
export type JSONSchemaStringProperties =
  | "description"
  | "default"
  | "maxLength"
  | "minLength"
...
export type JSONSchemaObjectProperties =
  | "title"
  | "description"
  | "maxProperties"
  | "minProperties"
...
export type JSONSchemaGeneralProperties =
  | "title"
  | "description"
  | "default"
  | "examples";
