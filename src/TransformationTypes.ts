 ...
 * type from T[P] | U[P].
 */
export type Merge<T, U> = Omit<T, Extract<keyof T, keyof U>> &
 ...
 */
export type PartialProperties<T, K extends keyof T> = Partial<Pick<T, K>> &
  Omit<T, K>;
...
> = {} extends Pick<T, K> ? RenameOptional<T, K, K2> : RenameRequired<T, K, K2>;

/**
 * T with property K renamed to K2 with required modifier
 */
export type RenameRequired<T, K extends keyof T, K2 extends keyof any> = Omit<
  T,
  K
> &
  { [P in K2]: T[K] };
 ...
 */
export type RenameOptional<T, K extends keyof T, K2 extends keyof any> = Omit<
  T,
  K
> &
  { [P in K2]?: T[K] };
 ...
 */
export type TransformProperties<T, K extends keyof T, U> = Omit<T, K> &
  { [P in K]: T[P] | U };

/**
 * T with properties K Transformed to T[P] | T[P][] only if T[P] is not already an Array
 */
export type TransformPropertiesToArray<T, K extends keyof T> = Omit<T, K> &
  { [P in K]: T[P] extends any[] ? T[P] : T[P] | T[P][] };

/**
 * T with properties K Transformed to A | T[P] only if T[P] is A[]
 */
export type UnwrapArrayProperties<T, K extends keyof T> = Omit<T, K> &
 ...
 * It allows the type to workaround circular refs limitations
 */
export type ArrayToOneOfObject<T> = T extends [...infer R]
  ? { t: OneOf<R> }
  : any;

export type OneOf<T> = T extends [
  SchemaBuilder<any>,
  SchemaBuilder<any>,
  ...any
]
 ...
 * It allows the type to workaround circular refs limitations
 */
export type ArrayToAllOfObject<T> = T extends [...infer R]
  ? { t: AllOf<R> }
  : any;
